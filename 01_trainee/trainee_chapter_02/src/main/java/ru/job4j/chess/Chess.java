...
public class Chess extends Application {
    ...
    private Rectangle buildFigure(int x, int y, int size, String image) {
        ...
        rect.setOnMouseDragged(
                event -> {
                    rect.setX(event.getX() - size / 2.0);
                    rect.setY(event.getY() - size / 2.0);
                }
        );
        rect.setOnMouseReleased(
                event -> {
                    if (logic.move(this.findBy(momento.getX(), momento.getY()), this.findBy(event.getX(), event.getY()))) {
                        rect.setX(((int) event.getX() / 40.0) * 40 + 5);
                        rect.setY(((int) event.getY() / 40.0) * 40 + 5);
                    } else {
                        rect.setX(((int) momento.getX() / 40.0) * 40 + 5);
                        rect.setY(((int) momento.getY() / 40.0) * 40 + 5);
                    }
                }
        );
        return rect;
